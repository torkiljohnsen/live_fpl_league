...
- **Tier 0** (every morning, 05:00–08:00 Oslo) → refreshes dashboards unconditionally. FPL keeps revising points after the last fixture flips to finished — bonus lands late and the classic-league standings endpoint refreshes on FPL's own batch schedule — so a fixture-count delta alone leaves the published standings a day behind. A three-hour window rather than one hour because scheduled runs drift and GitHub may drop one; rendering is deterministic, so the later runs find no diff and commit nothing.
