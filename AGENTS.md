...
6. **Narrative Generation** - [`fpl/narrative_generator.py`](fpl/narrative_generator.py) sends report + Reidar persona/guide/examples/device palette + memory context to Claude API, returns Norwegian-language narrative. The user message also carries this week's scheduled shape ([`fpl/format_scheduler.py`](fpl/format_scheduler.py), seeded by season+GW), on-demand reference docs ([`fpl/reference_loader.py`](fpl/reference_loader.py)) and a do-not-repeat block from memory; the draft is checked by [`fpl/style_lint.py`](fpl/style_lint.py) and regenerated once if a hard limit breaks. The request itself goes through [`fpl/claude_api.py`](fpl/claude_api.py), which holds the model choice and `max_tokens` for both the narrative and the memory update, and raises on a refusal or a truncated response. `run_narrative_pipeline()` orchestrates the full flow.
7. **Memory Update** - [`fpl/reidar_memory.py`](fpl/reidar_memory.py) updates per-manager profiles, season arc, GW summaries, the prediction ledger and open threads after each narrative; `recent.json` (headlines/openers/closers/jokes) and `shapes.json` are maintained by code
8. **Teams Notification** - [`fpl/teams_notification.py`](fpl/teams_notification.py) posts an Adaptive Card with the front-matter teaser (`teaser:`/`mentions:` written by Reidar), a rotated image and link label
...
**Reidar Memory System**: Persistent context across gameweeks stored in `weekly_report/reidar_memory/{league_id}/{season}/`. Includes per-manager profiles (~200 words), season arc, and rolling GW summaries. `league_context.md` is hand-maintained (who works where, off-pitch facts): always in the prompt, never written by the memory update. Assembled into prompt context via `ReidarMemory.get_prompt_context()` (~4k words at any point in the season).
...
- **Tier 0** (every morning, 04:00–08:00 Oslo) → refreshes dashboards unconditionally. FPL keeps revising points after the last fixture flips to finished — bonus lands late and the classic-league standings endpoint refreshes on FPL's own batch schedule — so a fixture-count delta alone leaves the published standings a day behind. The dashboard must be current by 06:00, before the 07:00 start of the working day; GitHub's scheduler is best-effort (runs land 10–30 min late, and one can be dropped), so the window opens at 04:00 for two attempts before the deadline. Rendering is deterministic, so the later runs find no diff and commit nothing.
