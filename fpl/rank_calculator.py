...
class RankCalculator:
    ...
    def apply_history_ranks(participants: list[Participant]) -> None:
        ...
        event_min_points: dict[Any, Any] = df.groupby("event")["net_points"].min().to_dict()
        event_max_points: dict[Any, Any] = df.groupby("event")["net_points"].max().to_dict()

        for p in participants:
            entry_id = p.entry_id
            p_hist = df[df["entry_id"] == entry_id]
            for h, row_data in zip(p.history, p_hist.to_dict(orient="records"), strict=True):
